...
public:
	...
	// 将时间转换成分钟数, uTime: 当前时间,格式如0910, autoAdjust: 是否自动调整,如果开启,非交易时间内的行情,会自动对齐到下一个交易时间,如8点59分的行情,会自动算作9点00的行情
	...
	// 是否处于交易时间, uTime: 时间(格式为hhmm), bStrict: 是否严格检查, 如果是严格检查则在每一交易时段最后一分钟(如1500)不属于交易时间
